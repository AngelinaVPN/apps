...
- flclashx-settings: Управление настройками приложения через хедер (с возможностью переопределения со стороны клиента), такими как: Сворачивать при выходе, Автозапуск приложения, Скрытый запуск, Автозапуск прокси, Автопроверка обновлений. Если вы не передаёте какой-то параметр, то этот параметр будет выключен.
